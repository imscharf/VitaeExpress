  ...
  <div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
    ...
    <div v-else>
      <div class="flex justify-between items-center mb-6">
        ...
        <div>
          <button @click="exportToPDF" class="bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700 transition mr-2">
            Baixar PDF
          </button>
      ...
      <form @submit.prevent="saveCV" class="space-y-8">
        <div class="p-6 bg-white rounded-lg shadow">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input v-model="cv.email" type="email" placeholder="E-mail de Contato" class="border rounded p-2 w-full" required />
            <input v-model="cv.phone" v-maska data-maska="(##) #####-####" placeholder="Telefone" class="border rounded p-2 w-full" />
        ...
        <div class="p-6 bg-white rounded-lg shadow">
          ...
          <textarea v-model="cv.professionalSummary" placeholder="Fale um pouco sobre sua carreira..." class="border rounded p-2 w-full h-24"></textarea>
        </div>
        ...
        <div class="p-6 bg-white rounded-lg shadow">
          ...
          <div v-for="(exp, index) in cv.experiences" :key="index" class="space-y-3 mb-4 border p-4 rounded">
            ...
            <input v-model="exp.period" placeholder="Período (Ex: Jan 2020 - Dez 2022)" class="border rounded p-2 w-full" />
            <textarea v-model="exp.description" placeholder="Descrição das atividades" class="border rounded p-2 w-full h-20"></textarea>
            <button @click.prevent="removeExperience(index)" class="text-red-500 hover:text-red-700">Remover Experiência</button>
          </div>
          <button @click.prevent="addExperience" class="mt-2 text-blue-600 hover:text-blue-800">+ Adicionar Experiência</button>
        </div>
        ...
        <div class="p-6 bg-white rounded-lg shadow">
          ...
          <div v-for="(edu, index) in cv.education" :key="index" class="space-y-3 mb-4 border p-4 rounded">
            ...
            <button @click.prevent="removeEducation(index)" class="text-red-500 hover:text-red-700">Remover Formação</button>
          </div>
          <button @click.prevent="addEducation" class="mt-2 text-blue-600 hover:text-blue-800">+ Adicionar Formação</button>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="p-6 bg-white rounded-lg shadow">
                <h2 class="text-xl font-semibold mb-4 border-b pb-2">Habilidades</h2>
                <textarea v-model="skillsAsText" placeholder="Liste suas habilidades separadas por vírgula (Ex: JavaScript, Vue.js, Liderança)" class="border rounded p-2 w-full h-24"></textarea>
            </div>
            <div class="p-6 bg-white rounded-lg shadow">
                <h2 class="text-xl font-semibold mb-4 border-b pb-2">Links</h2>
                <input v-model="cv.links.linkedIn" placeholder="URL do seu LinkedIn" class="border rounded p-2 w-full mb-2" />
      ...
      <div class="mt-8 p-6 bg-white rounded-lg shadow">
        ...
        <div class="flex flex-wrap gap-4">
          <button @click="share('linkedin')" class="bg-[#0077b5] text-white py-2 px-4 rounded hover:opacity-90 transition-opacity">LinkedIn</button>
          <button @click="share('twitter')" class="bg-[#1DA1F2] text-white py-2 px-4 rounded hover:opacity-90 transition-opacity">Twitter</button>
          <button @click="share('whatsapp')" class="bg-[#25D366] text-white py-2 px-4 rounded hover:opacity-90 transition-opacity">WhatsApp</button>
        </div>
      </div>
    </div>
  </div>
